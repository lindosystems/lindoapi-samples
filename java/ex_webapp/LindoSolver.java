...
public class LindoSolver extends Lindo
{
    ...
    private static void APIErrorCheck(Object /*long*/ pEnv ) throws Exception
    {
        if(0 != nErrorCode[0])
        {
            ...
            _log.debug("");
            throw (new Exception("Error " + nErrorCode[0] + ": " + cErrorMessage));
        }
    }
    ...
    public static void run (LindoSolver ls, Logger log) throws Exception
    {
        ...
        nErrorCode[0] = LSloadLicenseString("../../license/lndapi130.lic",cLicenseKey);
        ...
        for (i = 0; i < nM; i++) log.debug(connames[i] + "\t" + adY[i] + "\t" + adS[i]);
        System.out.print("\n");
	...
	public static void solveFile(LindoSolver ls, Logger log, String szFile) throws Exception
	{
		_log = log;
		int i,neq,nle,nge;
		int m[] = new int[1];
		...
		cLicenseKey.setLength(0);
        nErrorCode[0] = LSloadLicenseString("d:/usr/Lindoapi/8.0/license/lndapi130.lic",cLicenseKey);
		try { APIErrorCheck(pEnv); } catch (Exception ex) { throw (ex);}
		...
		{
			log.debug("Failed..");
			szOut = String.format("Reading %s as LINDO formatted file. ",szFile);
			log.debug(szOut);
		...
		verbose = 2;
		//nErrorCode[0] = LSsetModelIntParameter(pModel,LS_IPARAM_LP_PRELEVEL,0);
		//nErrorCode[0] = LSsetModelIntParameter(pModel,LS_IPARAM_LP_PRINTLEVEL,verbose);
		try { APIErrorCheck(pEnv); } catch (Exception ex) { throw (ex);}
		...
		else
		{
			...
			nErrorCode[0] = LSgetKBestMIPSols( pModel,"LindoSolver/nextmip.sol","jNextMIPCallback",ls,10);
			try { APIErrorCheck(pEnv); } catch (Exception ex) { throw (ex);}
		}
		log.debug("Done!");
