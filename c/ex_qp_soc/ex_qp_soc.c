...
###################################################################
  ...
  File   : ex_qp_soc.c
  Purpose: Solve a mixed convex QP and second-order cone program.
  ...
  Q-matrix for the cone constraint:
            w     x1    x2    x3    x4   x5
         [ -2.00  0.    0.    0.    0.   0.  ]
         [  0.    1.00  0.    0.    0.   0.  ]
     Q = [  0.    0.    1.00  0.    0.   0.  ]
...
int main()
{
  ...
  /*****************************************************************
   * Step 4: Specify the quadratic matrix to characterize the QCONE
   *****************************************************************/
      ...
      int paiQCcols1[1] = {0} ;
      int paiQCcols2[1] = {0} ;
      double padQCcoef[1] = {2.0} ;

     /* The number of nonzeros in the quadratic matrix */
      int nCones = 1;

      /* Specify the col indices of the nonzero coefficients in the
         Q-matrix. */
      ...
      * by a call to LSloadQCData()  */
      nErrorCode = LSloadQCData(pModel, nQCnnz, paiQCrows,
        paiQCcols1, paiQCcols2, padQCcoef);
      APIERRORCHECK;


     /* Pass Cone data to problem structure
      * by a call to LSloadConeData()  */
      nErrorCode = LSloadConeData(pModel, nCones, pszConeTypes, padConeAlpha,
        paiConebeg, paiConecols);
      APIERRORCHECK;

      /* export model in MPS format */
      LSwriteMPSFile(pModel,"QP_cone.mps",0);
   ...
   *****************************************************************/
   if (nSolStatus == LS_STATUS_OPTIMAL || nSolStatus == LS_STATUS_BASIC_OPTIMAL)
   {
      ...
      printf("Minimum objective = %11.5f*\n",dObj);
      for (i = 0; i < nN; i++)
        printf("%7s x[%d] = %11.5f\n","",i,x[i] );
      printf ("\n");
   }
