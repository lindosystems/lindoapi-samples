...
/* We have now assembled a full description of the model.
 ...
 /* >>> Step 5 <<< Retrieve the solution */
      int i;
      double padPrimals[ 4], dObj;
      double padDuals[ 4];
      double padRedcosts[ 4];
      double padSlacks[ 4];
      int    panCstatus[4], panRstatus[4];
 ...
 /* Get the variable values */
	  ...
	  for (i=0; i<nCons; i++)
	  {
       ...
       nErrorCode = LSwriteLINDOFile(pModel,"ex_lp2.ltx");
       APIERRORCHECK;
       nErrorCode = LSwriteMPSFile(pModel,"ex_lp2.mps",LS_UNFORMATTED_MPS);
       APIERRORCHECK;
       nErrorCode = LSwriteLINGOFile(pModel,"ex_lp2.lng");
       APIERRORCHECK;
