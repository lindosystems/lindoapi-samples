...
int  LS_CALLTYPE print_log(pLSmodel model,int iLoc, void *cbData)
{
  ...
  else if ( iLoc == LSLOC_GOP)
  {
    ...
    LSgetCallbackInfo(model,iLoc,LS_IINFO_GOP_SIM_ITER,&i); iter+=i;
    LSgetCallbackInfo(model,iLoc,LS_IINFO_GOP_BAR_ITER,&i); iter+=i;
    LSgetCallbackInfo(model,iLoc,LS_IINFO_GOP_NLP_ITER,&i); iter+=i;
