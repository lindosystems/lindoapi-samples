...
###################################################################
  ...
  File   : ex_nlp2_lin.c
  Purpose: Reformulate and solve the same example in ex_nlp2 to
           allow full linearization. The reformulation involves
           a simple expansion of the parentheses in constraint #6.
  ...
  Model  : A nonlinear model with multiple local optima.
            ...
            s.t.     x0           + x1 - 4      <= 0;
                     ...
                     max(x0 , x1 + 1)           >= 0;
                     if(x1, 1, x1)              <= 0;
                     x1 * 2 * x1 * x0 - x1* x0  <= 0;   (Constraint #6)
                     -100  <=  x0  <=  100
                     x1 is binary
 ...
 */
  ...
  icon++;
      ...
      /*
       *  Reformulated instruction code of constraint 5:
       *
       *  x1 * 2 * x1 * x0 -  x1 * x0      <= 0;
       */
       /* Constraint type */
